...
void InputSetup( string &NAME, string &OUTPUTMOD, float &x_start, float &x_end, float &y_start,
  ...
  UE           = all_numbers[18];
  CORE_OUT     = all_numbers[19];
  MATT_OUT     = all_numbers[20];
...
void CreateCones( float* conelist_1D , string input,long unsigned &CONES, float UE ){ // Creating the list of cones by importing from DATAFILE
  ...
  float KN; // The first part of the Klein-Nishina coefficient
  unsigned errors = 0;
