...
int main() {
  ...
  for (unsigned It = 1; It <= TOTALIT; It++) {
    ...
    if (It%SAVEEVERY == 0 || It == 1 || It == TOTALIT ) {
      cudaMemcpy(f, f_d, XDIVI*YDIVI*ZDIVI*sizeof(float), cudaMemcpyDeviceToHost);
      string output =  "../Output/" + NAME + "_" + OUTPUTMOD + "_C" + std::to_string(CONES) + "_x" + std::to_string(XDIVI) + "y" + std::to_string(YDIVI) + "z" + std::to_string(ZDIVI) + "_I"; // Output file name
