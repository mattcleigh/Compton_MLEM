...
void InputSetup( string &NAME, string &OUTPUTMOD, float &x_start, float &x_end, float &y_start,
                ...
                unsigned &INTSTEP, float &CUTOFF, bool &CORE_OUT, bool &MATT_OUT ){
  ...
  INTSTEP      = all_numbers[16];
  CUTOFF       = all_numbers[17];
...
int MemDebugger( unsigned XDIVI, unsigned YDIVI, unsigned ZDIVI, long unsigned CONES ){
  ...
  size_t matrix_mem = XDIVI * YDIVI * ZDIVI * CONES * sizeof(unsigned char);
...
void DefinePositions( vector<vector<vector<vector<float> > > > &position_matrix, float* f, unsigned XDIVI, unsigned YDIVI, unsigned ZDIVI,
  ...
  cout << "Defining lattice Positions:\n";

  for (unsigned i = 0; i < XDIVI; i++) {
    for (unsigned j = 0; j < YDIVI; j++) {
      for (unsigned k = 0; k < ZDIVI; k++) {

        position_matrix[i][j][k] = { x_start + delx * (float)(i+0.5) , y_start + dely * (float)(j+0.5) , z_start + delz * (float)(k+0.5) };

        f[ i + j * XDIVI + k * XDIVI * YDIVI ] = 1.0; // Initial guess for f is one everywhere
...
void CreateCones( float* conelist_1D , string input, long unsigned CONES  ){ // Creating the list of cones by importing from DATAFILE
  ...
  for (unsigned i = 0; i < CONES; i++) {
    ...
    vector<float> axis =  unit_vector ( { linedata[3] , linedata[4] , linedata[5] } , { linedata[0] , linedata[1] , linedata[2] } );
    //The axis is the unit vector which points in the direction from the second scatter to the first

    conelist_1D [ 0 + i * 11 ] = linedata[0]; // First scattering location
    conelist_1D [ 1 + i * 11 ] = linedata[1];
    conelist_1D [ 2 + i * 11 ] = linedata[2];

    conelist_1D [ 3 + i * 11 ] = axis[0]; // Axis of the cone
    conelist_1D [ 4 + i * 11 ] = axis[1];
    conelist_1D [ 5 + i * 11 ] = axis[2];

    conelist_1D [ 6 + i * 11 ] = linedata[6]; // Scattering angle
    conelist_1D [ 7 + i * 11 ] = linedata[7]; // Scattering angle uncertainty

    conelist_1D [ 8 + i * 11 ] = linedata[8]; // First part of the Klein-Nishina coefficient

    conelist_1D [ 9 + i * 11 ] = linedata[9]; // First energy deposition
    conelist_1D [ 10 + i * 11 ] = linedata[10]; // Second energy deposition

    linedata.clear();
  }
  cout << "Data has been imported and Cones have been created" << '\n';
}
